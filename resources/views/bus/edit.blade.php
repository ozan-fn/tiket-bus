<x-admin-layout>
                ...
                <div id="tab-informasi" class="tab-content space-y-6">
                    <x-ui.card>
                        <x-ui.card.header>
                            ...
                            <x-ui.card.description>Perbarui detail identitas bus</x-ui.card.description>
                        </x-ui.card.header>
                        <x-ui.card.content>
                            <div class="space-y-6">
                                <!-- Nama Bus -->
                            ...
                            <div class="space-y-4">
                                ...
                                >
                                    @foreach($fasilitas as $fasilitasItem)
                                        <option value="{{ $fasilitasItem->id }}" {{ $bus->fasilitas->contains($fasilitasItem->id) ? 'selected' : '' }}>
                                            {{ $fasilitasItem->nama }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="mt-6 space-y-3">
                                    <h4 class="text-sm font-semibold">Fasilitas Terpilih:</h4>
                    ...
                    <x-ui.card>
                        ...
                        <x-ui.card.content>
                            <div id="kelas-bus-container" class="space-y-4">
                                <!-- Existing Kelas Bus -->
                                @foreach($bus->kelasBus as $idx => $kelas)
                                    <div class="kelas-bus-row flex gap-3 items-end p-4 rounded-lg border border-border bg-muted/30 hover:bg-muted/50 transition-colors">
                                        <div class="flex-1">
                                            ...
                                            <select name="kelas_bus_data[{{ $idx }}][kelas_id]" class="kelas-select w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
                                                <option value="">-- Pilih Kelas --</option>
                                                @foreach($kelasBus as $kb)
                                                    <option value="{{ $kb->id }}" {{ $kb->id == $kelas->id ? 'selected' : '' }}>{{ $kb->nama_kelas }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="flex-1">
                                            ...
                                            <x-ui.input
                                                type="number"
                                                name="kelas_bus_data[{{ $idx }}][jumlah_kursi]"
                                                placeholder="Masukkan jumlah kursi"
                                                value="{{ $kelas->pivot->jumlah_kursi }}"
                                                min="1"
                                                class="kelas-kursi"
                                            />
                                        </div>
                            ...
                            <div class="mt-6 p-4 rounded-lg bg-muted/50 border border-border">
                                ...
                                <div id="kelas-summary" class="text-sm text-muted-foreground space-y-2">
                                    @if($bus->kelasBus->count() > 0)
                                        @foreach($bus->kelasBus as $kelas)
                                            <div class="flex justify-between items-center p-2 bg-background rounded border border-border">
                                                ...
                                                <x-ui.badge variant="secondary">{{ $kelas->pivot->jumlah_kursi }} Kursi</x-ui.badge>
                                            </div>
                                        @endforeach
                                    @else
                ...
                <div id="tab-foto" class="tab-content hidden space-y-6">
                    <x-ui.card>
                        <x-ui.card.header>
                            ...
                            <x-ui.card.description>Unggah atau kelola foto bus</x-ui.card.description>
                        </x-ui.card.header>
                        <x-ui.card.content>
                            <div class="space-y-4">
                                <!-- Existing Photos -->
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                            @foreach($bus->photos as $photo)
                                                <div class="relative group">
                                                    <img src="{{ asset('storage/' . $photo->path) }}" alt="Bus Photo" class="w-full h-24 object-cover rounded-lg border border-border">
                                                    <button
                                                        type="button"
    ...
    @push('scripts')
    <script>
        let kelasBusCount = {{ $bus->kelasBus->count() }};
        ...
        function deleteFoto(photoId, element) {
            if (!confirm('Hapus foto ini?')) return;

            fetch(`/admin/bus-photo/${photoId}`, {
                method: 'DELETE',
                headers: {
