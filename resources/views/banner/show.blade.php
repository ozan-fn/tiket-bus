                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <div class="h-16 w-16 rounded-lg bg-primary/10 flex items-center justify-center">
                                <i data-lucide="image" class="h-8 w-8 text-primary" ></i>
                            </div>
                            <div>
                                <x-ui.card.title class="text-2xl">{{ $banner->title }}</x-ui.card.title>
                                <x-ui.card.description>{{ $banner->description ?? 'Tidak ada deskripsi' }}</x-ui.card.description>
                            </div>
                        </div>
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            <a href="{{ route('admin/banner.edit', $banner) }}" class="flex-1 sm:flex-initial">
                ...
                <x-ui.card.content>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
                            ...
                            <div>
                                <p class="text-sm text-muted-foreground">ID Banner</p>
                                <p class="text-2xl font-bold">#{{ $banner->id }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
                            <div class="h-10 w-10 rounded-lg bg-orange-500/10 flex items-center justify-center shrink-0">
                                <i data-lucide="list-ordered" class="h-5 w-5 text-orange-600 dark:text-orange-400" ></i>
                            </div>
                            <div>
                                <p class="text-sm text-muted-foreground">Urutan</p>
                                <p class="text-2xl font-bold">{{ $banner->order }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
                            ...
                            <div>
                                ...
                                <p class="text-xl font-bold">{{ $banner->owner->name }}</p>
                            </div>
                        </div>
            ...
            <x-ui.card>
                <x-ui.card.header>
                    <div class="flex items-center gap-2">
                        ...
                        <x-ui.card.title>Gambar Banner</x-ui.card.title>
                    </div>
                    <x-ui.card.description>Gambar yang digunakan untuk banner</x-ui.card.description>
                </x-ui.card.header>
                <x-ui.card.content>
                    @if($banner->image)
                        <div class="relative group aspect-video max-w-md rounded-lg overflow-hidden border border-border">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-sm text-muted-foreground">Tidak ada gambar</p>
                        </div>
                    @endif
                </x-ui.card.content>
            </x-ui.card>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium">{{ $banner->created_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium">{{ $banner->updated_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </x-ui.card.content>
