                    ...
                    <div class="flex-1">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <x-ui.card.description>Semua kelas bus yang tersedia</x-ui.card.description>
                            </div>
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="flex gap-2 items-center w-full">
                                <form method="GET" action="{{ route('admin/kelas-bus.index') }}"
                                    ...
                                    <div class="flex flex-row gap-2">
                                        <div class="relative flex-1 min-w-0">
                                            ...
                                            <x-ui.input type="text" name="search" placeholder="Cari kelas bus..."
                                                value="{{ $search ?? '' }}" class="pl-9 h-10 max-w-md" />
                                        </div>
                                ...
                                <!-- Table Controls -->
                                <div class="shrink-0">
                                    <x-table.controls :search="$search" :sort="$sort" :sortField="$sortField"
                                        :order="$order" :dateFrom="$dateFrom" :dateTo="$dateTo" />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <x-ui.table>
                            <x-ui.table.header class="hidden sm:table-header-group">
                                <x-ui.table.row>
                                    ...
                                    <x-table.sortable-header name="created_at" class="hidden md:table-cell">Tanggal
                                        Dibuat</x-table.sortable-header>
                            ...
                            <x-ui.table.body>
                                @foreach($kelasBus as $index => $item)
                                    ...
                                    <x-ui.table.row class="hidden sm:table-row">
                                        <x-ui.table.cell
                                            class="font-medium text-xs sm:text-sm">{{ $kelasBus->firstItem() + $index }}</x-ui.table.cell>
                                        <x-ui.table.cell>
                                            <div class="flex items-center gap-2 sm:gap-3">
                                                <div
                                                    class="h-8 w-8 sm:h-10 sm:w-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                                        ...
                                        <x-ui.table.cell class="text-right">
                                            <div class="flex items-center justify-end gap-1">
                                                ...
                                                <div x-data="{ open: false }">
                                                    <x-ui.button @click="open = true" variant="ghost" size="sm"
                                                            ...
                                                            <div class="flex min-h-full items-center justify-center p-4">
                                                                <div x-show="open"
                                                                    ...
                                                                    <div class="flex items-start gap-4">
                                                                        ...
                                                                        <div class="flex-1">
                                                                            <h3 class="text-lg font-semibold mb-2">Hapus Kelas Bus
                                                                            </h3>
                                                                            <p class="text-sm text-muted-foreground mb-4">
                                                                                Apakah Anda yakin ingin menghapus kelas bus
                                                                                <strong>{{ $item->nama_kelas }}</strong>?
                                                                                Tindakan ini tidak dapat dibatalkan.
                                                                            </p>

                                                                            <div
                                                                                class="flex flex-col-reverse sm:flex-row gap-2 justify-end">
                                                                                <x-ui.button type="button" variant="outline"
                                                                                    @click="open = false" size="sm">
                                                                                    Batal
                                                                                </x-ui.button>
                                                                                <form method="POST"
                                                                                    action="{{ route('admin/kelas-bus.destroy', $item) }}"
                                                                                    class="inline">
                                                                                    @csrf
                                                                                    @method('DELETE')
                                                                ...
                                                                <div class="flex min-h-full items-center justify-center p-4">
                                                                    <div x-show="open"
                                                                        ...
                                                                        <div class="flex items-start gap-4">
                                                                            ...
                                                                            <div class="flex-1">
                                                                                <h3 class="text-lg font-semibold mb-2">Hapus Kelas
                                                                                    Bus</h3>
                                                                                <p class="text-sm text-muted-foreground mb-4">Apakah
                                                                                    Anda yakin ingin menghapus kelas bus
                                                                                    <strong>{{ $item->nama_kelas }}</strong>?
                ...
                <x-ui.card.footer>
                    <div class="w-full flex flex-col sm:flex-row items-center justify-between gap-3">
                        <p class="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
                            Menampilkan {{ $kelasBus->firstItem() }} - {{ $kelasBus->lastItem() }} dari {{ $kelasBus->total() }}
                            kelas bus
                        </p>
