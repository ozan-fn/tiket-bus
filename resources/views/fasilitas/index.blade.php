        ...
        <x-ui.card>
            <x-ui.card.header>
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div class="flex-1">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <x-ui.card.description>Semua fasilitas bus yang tersedia</x-ui.card.description>
                            </div>
                            <a href="{{ route('admin/fasilitas.create') }}" class="hidden sm:inline-block">
                                <x-ui.button>
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="flex gap-2 items-center w-full">
                                <form method="GET" action="{{ route('admin/fasilitas.index') }}" class="flex gap-2 flex-1 min-w-0">
                                    ...
                                    @if($search)
                                        <a href="{{ route('admin/fasilitas.index') }}" class="shrink-0">
                    ...
                    <!-- Mobile Add Button -->
                    <a href="{{ route('admin/fasilitas.create') }}" class="sm:hidden w-full">
                        ...
                        <x-ui.table>
                            <x-ui.table.header class="hidden sm:table-header-group">
                                <x-ui.table.row>
                                    ...
                                    <x-table.sortable-header name="nama">Nama Fasilitas</x-table.sortable-header>
                            ...
                            <x-ui.table.body>
                                @foreach($fasilitas as $index => $item)
                                    ...
                                    <x-ui.table.row class="hidden sm:table-row">
                                        <x-ui.table.cell class="font-medium text-xs sm:text-sm">{{ $fasilitas->firstItem() + $index }}</x-ui.table.cell>
                                        <x-ui.table.cell>
                                            <div class="flex items-center gap-2 sm:gap-3">
                                                <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                                                    <i data-lucide="sparkles" class="h-4 w-4 sm:h-5 sm:w-5 text-primary" ></i>
                                                </div>
                                                <div class="min-w-0">
                                                                    ...
                                                                    <div class="flex items-start gap-4">
                                                                        ...
                                                                        <div class="flex-1">
                                                                            ...
                                                                            <div class="flex flex-col-reverse sm:flex-row gap-2 justify-end">
                                                                                ...
                                                                                <form method="POST" action="{{ route('admin/fasilitas.destroy', $item) }}" class="inline">
                                                                                    @csrf
                                                                                    @method('DELETE')
                                                                ...
                                                                <div class="flex min-h-full items-center justify-center p-4">
                                                                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" @click.stop class="relative w-full max-w-lg bg-card rounded-lg shadow-lg border border-border p-6">
                                                                        <div class="flex items-start gap-4">
                                                                            ...
                                                                            <div class="flex-1">
                                                                                <h3 class="text-lg font-semibold mb-2">Hapus Fasilitas</h3>
                                                                                <p class="text-sm text-muted-foreground mb-4">Apakah Anda yakin ingin menghapus fasilitas <strong>{{ $item->nama }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
                                                                                <div class="flex flex-col-reverse sm:flex-row gap-2 justify-end">
                ...
                @else
                    <div class="text-center py-12">
                        <div class="h-16 w-16 rounded-full bg-muted flex items-center justify-center mx-auto mb-4">
                            ...
                            @else
                                <i data-lucide="sparkles" class="h-8 w-8 text-muted-foreground" ></i>
                            @endif
                        </div>
