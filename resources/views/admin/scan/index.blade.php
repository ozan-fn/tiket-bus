                                ...
                                <template x-if="result?.data">
                                    <div class="space-y-4 pt-2 border-t border-border/50">
                                        ...
                                        <div class="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p class="font-bold text-sm text-primary"
                                                    x-text="result.data.kode_tiket"></p>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-[10px] text-muted-foreground uppercase">Status</p>
                                                <span
                                                    :class="result.data.status === 'dibayar' ? 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800' : (result.data.status === 'dipesan' ? 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800' : (result.data.status === 'selesai' ? 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800' : (result.data.status === 'batal' ? 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800' : 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800')))"
                                                    x-text="result.data.status || 'N/A'"></span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <p class="font-bold text-sm"
                                                    x-text="result.data.nama_penumpang || 'N/A'"></p>
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <p class="font-bold text-sm"
                                                    x-text="result.data.jenis_kelamin === 'L' ? 'Laki-laki' : (result.data.jenis_kelamin === 'P' ? 'Perempuan' : 'N/A')">
                                                </p>
                                            </div>
                                            <div>
                                                <p class="text-[10px] text-muted-foreground uppercase">Nomor Telepon</p>
                                                <p class="font-bold text-sm"
                                                    x-text="result.data.nomor_telepon || 'N/A'"></p>
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <p class="font-bold text-sm text-primary"
                                                    x-text="result.data.kursi || 'N/A'"></p>
                                            </div>
                                        </div>

                                        <!-- Schedule Info -->
                                        <div class="p-3 bg-background/50 rounded-lg border">
                                            <p class="text-[10px] text-muted-foreground uppercase mb-2 text-center">
                                                Informasi Perjalanan</p>
                                            <div class="space-y-2">
                                                <div class="flex justify-between">
                                                    <span class="text-xs text-muted-foreground">Tanggal:</span>
                                                    <span class="font-bold text-xs"
                                                        x-text="result.data.jadwal && result.data.jadwal.tanggal_berangkat ? result.data.jadwal.tanggal_berangkat.split('T')[0] : 'N/A'"></span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-xs text-muted-foreground">Jam:</span>
                                                    <span class="font-bold text-xs"
                                                        x-text="result.data.jadwal && result.data.jadwal.jam_berangkat ? result.data.jadwal.jam_berangkat.split('T')[1].split('.')[0] : 'N/A'"></span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-xs text-muted-foreground">Kelas:</span>
                                                    <span class="font-bold text-xs"
                                                        x-text="result.data.kelas || 'N/A'"></span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-xs text-muted-foreground">Harga:</span>
                                                    <span class="font-bold text-xs text-green-600"
                                                        x-text="result.data.harga ? 'Rp ' + result.data.harga.toLocaleString('id-ID') : 'N/A'"></span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="p-3 bg-primary/5 rounded-lg border border-primary/20">
                                            <p class="text-[10px] text-muted-foreground uppercase mb-1 text-center">Rute
                                                Perjalanan</p>
                                            <div class="flex justify-between items-center px-4">
                                                <span class="font-bold text-sm"
                                                    x-text="result.data.jadwal && result.data.jadwal.asal ? result.data.jadwal.asal : 'N/A'"></span>
                                                <i data-lucide="arrow-right" class="w-4 h-4 text-primary"></i>
                                                <span class="font-bold text-sm"
                                                    x-text="result.data.jadwal && result.data.jadwal.tujuan ? result.data.jadwal.tujuan : 'N/A'"></span>
                                            </div>
                                        </div>
                                    </div>
                                </template>
