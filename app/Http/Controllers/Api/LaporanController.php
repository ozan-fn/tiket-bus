...
class LaporanController extends Controller
{
    ...
    public function pendapatan(Request $request)
    {
        ...
        $query = Pembayaran::where('status', 'berhasil');
        ...
        if ($groupBy === 'monthly') {
            $data = $query->select(
                DB::raw('DATE_FORMAT(waktu_bayar, "%Y-%m") as periode'),
            ...
            $data = $query->select(
                DB::raw('DATE(waktu_bayar) as periode'),
                DB::raw('SUM(nominal) as total'),
                DB::raw('COUNT(*) as jumlah_transaksi')
            )
                ->groupBy('periode')
    ...
    {
        $query = Tiket::with(['jadwalKelasBus.jadwal.bus', 'jadwalKelasBus.jadwal.rute'])
        ...
        // Group by bus
        $perBus = $query->get()->groupBy(function ($tiket) {
        ...
        // Group by rute
        $perRute = $query->get()->groupBy(function ($tiket) {
