...
class KursiController extends Controller
{
    ...
    public function getKursiByJadwal($jadwalId, Request $request)
    {
        ...
        $jadwalKelasBus = \App\Models\JadwalKelasBus::with(["jadwal.bus", "busKelasBus.kelasBus", "busKelasBus.kursi"])->find($jadwalKelasBusId);

        if (!$jadwalKelasBus) {
            return response()->json(
                [
                    "success" => false,
                    "message" => "Jadwal kelas bus tidak ditemukan",
                ],
                404,
            );
        }
        ...
        // Order kursi by nomor_kursi ascending before mapping
        $kursiCollection = $jadwalKelasBus->busKelasBus->kursi->sortBy("nomor_kursi")->values();

        $kursiList = $kursiCollection->map(function ($kursi) use ($kursiTerpakai) {
            return [
                "id" => $kursi->id,
        ...
        return response()->json([
            ...
            "data" => [
                ...
                "kelas_bus" => [
                    "id" => $jadwalKelasBus->busKelasBus->kelasBus->id,
                    "nama_kelas" => $jadwalKelasBus->busKelasBus->kelasBus->nama_kelas,
                    "deskripsi" => $jadwalKelasBus->busKelasBus->kelasBus->deskripsi,
                    "jumlah_kursi" => $jadwalKelasBus->busKelasBus->jumlah_kursi,
                ],
