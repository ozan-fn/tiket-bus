...
class PembayaranController extends Controller
{
    ...
    /**
     * Upload bukti pembayaran manual (transfer/tunai)
     * POST /api/pembayaran/{id}/upload-bukti
     */
    public function uploadBukti(Request $request, $id)
    {
        $request->validate([
            "bukti_pembayaran" => "required|image|mimes:jpeg,png,jpg,gif|max:5120", // max 5MB
        ]);

        $pembayaran = Pembayaran::findOrFail($id);

        // Validasi hanya pembayaran manual (transfer/tunai) yang dapat upload bukti
        if (!in_array($pembayaran->metode, ["transfer", "tunai"])) {
            return response()->json(
                [
                    "success" => false,
                    "message" => "Hanya pembayaran transfer atau tunai yang dapat upload bukti",
                ],
                422,
            );
        }
        ...
        $path = $file->storeAs("pembayaran-bukti", "pembayaran_" . $pembayaran->id . "_" . time() . "." . $file->getClientOriginalExtension(), "public");
    ...
    /**
     * Get list pembayaran manual yang pending review
     * GET /api/pembayaran/manual/pending
     */
    public function getPembayaranManualPending()
    {
        ...
        return response()->json([
            ...
            "data" => $pembayarans->map(
                fn($pembayaran) => [
                    ...
                    "nominal" => $pembayaran->nominal,
                    "status" => $pembayaran->status,
                    "bukti_pembayaran" => $pembayaran->bukti_pembayaran ? url("storage/" . $pembayaran->bukti_pembayaran) : null,
                    "tiket" => [
                        "kode_tiket" => $pembayaran->tiket->kode_tiket,
                        "nama_penumpang" => $pembayaran->tiket->nama_penumpang,
                        "jadwal" => $pembayaran->tiket->jadwalKelasBus->jadwal->kode_jadwal,
                    ],
                    "user" => [
